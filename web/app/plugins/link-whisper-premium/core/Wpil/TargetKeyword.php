...
//                $data = self::process_custom_keywords($data, $start); // we don't have to process the custom keywords now since we move them when the reset is run. So they're never deleted
    ...
    public static function get_ai_generated_keywords_data(){
        ...
        $keyword_data = get_transient('wpil_target_keyword_ai_generated_ids');
        if(empty($keyword_data)){
            $keyword_data = array('posts' => false, 'terms' => false);
            ...
            // get the post ids & titles
            $post_data = $wpdb->get_results("SELECT a.ai_keyword_index, a.post_id, a.keywords FROM {$table} a LEFT JOIN {$wpdb->posts} b ON a.post_id = b.ID WHERE `keywords_loaded` < 1 $statuses_query $query_types");
            if(!empty($post_data)){
                $kw_data = array();
                foreach($post_data as $dat){
            ...
            // get the term ids
            $term_data = $wpdb->get_results("SELECT `ai_keyword_index`, `post_id`, `keywords` FROM {$table} WHERE `post_type` = 'term' AND `keywords_loaded` < 1");
...
//                continue;
    ...
    public static function ajax_create_custom_target_keyword(){
        ...
        if(!empty($inserted)){
            ...
            // note that the keywords have been created
            Wpil_Telemetry::log_event('custom_target_keyword_created', array('count' => count($keyword_data)));
    ...
    public static function ajax_delete_custom_target_keyword(){
        ...
        if($deleted){
            ...
            Wpil_Telemetry::log_event('custom_target_keyword_deleted');
