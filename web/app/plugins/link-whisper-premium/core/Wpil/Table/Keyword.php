...
class Wpil_Table_Keyword extends WP_List_Table
{
    ...
    function prepare_items()
    {
        ...
        if(empty($page) && empty($search) && empty($orderby) && empty($order)){
            Wpil_Telemetry::log_event('report_open_autolinking');
        }
                    ...
                    foreach($terms as $term){
                        if(isset($tax_cache[$term->taxonomy]) && !empty($tax_cache[$term->taxonomy]) && $tax_cache[$term->taxonomy]->hierarchical){
                            $cat_options .= '<li>
                                    ...
                                    <input type="checkbox" class="wpil-restrict-keywords-input" name="wpil_keywords_restrict_term_' . $term->term_taxonomy_id . '" ' . (in_array($term->term_taxonomy_id, $restricted_cats)?'checked':'') . ' data-term-id="' . $term->term_taxonomy_id . '">' . esc_html($term->name) . '</li>';
                        }else{
                            $tag_options .= '<li>
                                    <input type="hidden" name="wpil_keywords_restrict_term_' . $term->term_taxonomy_id . '" value="0" />
                                    <input type="checkbox" class="wpil-restrict-keywords-input" name="wpil_keywords_restrict_term_' . $term->term_taxonomy_id . '" ' . (in_array($term->term_taxonomy_id, $restricted_cats)?'checked':'') . ' data-term-id="' . $term->term_taxonomy_id . '">' . esc_html($term->name) . '</li>';
                ...
                return '<div class="wpil-autolink-rule-keyword">' . esc_html(stripslashes($item->$column_name)) . '<i class="dashicons dashicons-admin-generic"></i></div>
                        <div class="local_settings">
                            <div class="block" data-id="' . $item->id . '">
                                ...
                                <input type="checkbox" name="wpil_keywords_restrict_to_live" ' . (isset($item->restrict_to_live) && $item->restrict_to_live==1?'checked':'') . ' value="1" />
                                <label for="wpil_keywords_restrict_to_live">' . __('Only create internal links when target is published?', 'wpil') . '</label>
                                <br>
