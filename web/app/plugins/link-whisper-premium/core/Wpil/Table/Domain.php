    ...
    function prepare_items()
    {
        ...
        if(!isset($_REQUEST['paged']) && empty($search) && empty($orderby) && empty($order) && empty($show_untargeted)){
            if(isset($_REQUEST['domain_focus'])){
                Wpil_Telemetry::log_event('report_open_link_external_focus');
            }else{
        ...
        switch($column_name) {
            ...
            case 'links':
                ...
                if(isset($_REQUEST['domain_focus'])){
                    ...
                    $external_focus = '<span class="wpil-link-relation-indicator" style="position: absolute; left: 10px;'.$highlight.'">' . $external_link_emphasis_percent . '%</span>' . $fix;
                }

                return '<div class="wpil-collapsible-wrapper" data-wpil-collapsible-host="' . $item['host'] . '" data-wpil-collapsible-nonce="' . wp_create_nonce(wp_get_current_user()->ID . 'wpil-collapsible-nonce') . '">
  			                <div class="wpil-collapsible wpil-collapsible-static wpil-links-count ' . ((!empty($links)) ? 'wpil-collapsible-has-data': 'wpil-collapsible-no-data') . '">'.$external_focus.'<span class="wpil_ul">'.count($links).'</span></div>
  				            <div class="wpil-content">'.$list.'</div>
                            ' . $delete_bar . '
  				        </div>';
            default:
