    ...
    public static function ajax_get_post_suggestions(){
        ...
        if(isset($_POST['type']) && 'outbound_suggestions' === $_POST['type']){
            ...
            if(empty($suggested_post_ids)/* && !$ai_powered*/){ //TODO: Think about later and come up with a fallback option for when AI's not available
                ...
                $linked_post_ids = Wpil_Post::getLinkedPostIDs($post);
                $excluded_posts = array_merge($linked_post_ids, $ignored_category_posts);
                $ignore_all_posts = Wpil_Settings::getAllIgnoredPosts();
                if(!empty($ignore_all_posts)){
                    foreach($ignore_all_posts as $ignore_post){
                        $bits = explode('_', $ignore_post);
                        if(isset($bits[1]) && !empty($bits[1]) && isset($bits[0]) && $bits[0] === 'post'){
                            $excluded_posts[] = $bits[1];
                        }
                    }
                }
                ...
                foreach($suggested_posts as $suggested_post){
                    ...
                    if(!empty($post_embeddings) && isset($post_embeddings[$id]) && $post_embeddings[$id] < self::$ai_suggestion_threashold){
                        continue;
                    }
        ...
        foreach($ids as $post_id) {
            ...
            foreach($post_data['calculation'] as $sentence => &$relations){
                foreach($relations as $pid => $score){
                    if($score < self::$ai_suggestion_threashold){
                        unset($relations[$pid]);
                    }
                }
...
//            $unique_keywords = self::getPostUniqueKeywords($target_keywords, $process_key);
        ...
        foreach ($phrases as $key_phrase => $phrase) {
            ...
            foreach ($suggestions as $key => $suggestion) {
                ...
                if($suggestion['length'] < self::get_min_anchor_length() && !isset($suggestion['passed_target_keywords'])){
