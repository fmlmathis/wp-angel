    ...
    public static function addLinks($meta, $post_id, &$content)
    {
        ...
        if (!empty($beaver)) {
            ...
            foreach ($meta as $link) {
                ...
                $restrict_to_live   = isset($meta['keyword_data']) ? !empty($meta['keyword_data']->restrict_to_live): false;
                ...
                if($is_autolink){
                    ...
                    // if the link is being restricted to publish only posts
                    if($restrict_to_live){
                        // check if the post is indeed published
                        $post = Wpil_Post::getPostByLink($meta['keyword_data']->link);
                        if(!empty($post) && $post->type === 'post' && $post->getStatus() !== 'publish'){
                            // if it's not, skip to the next link
                            continue;
                        }
                    }
                }
