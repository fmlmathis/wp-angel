...
namespace LWVendor\ZendTech\Polyfill\MbEreg;
...
final class MbEreg
{
     ...
     * @internal
     */
    const REGEX_OPTIONS_MAP = ['i' => 'i', 'x' => 'x', 'm' => 'm', 's' => 's', 'p' => 'ms', 'l' => '', 'n' => '', 'e' => ''];
    /**
    ...
    public static function regexSetOptions($options = null)
    {
        ...
        for ($i = 0; $i < $optionsLength; $i += 1) {
            $option = $options[$i];
            if (\false !== strpos(self::REGEX_OPTIONS, $option) && \false === strpos($newOptions, $option)) {
                $newOptions .= $option;
                continue;
            }
            if (\false !== strpos(self::REGEX_MODES, $option) && $mode !== $option) {
                $mode = $option;
                continue;
            }
        }
     ...
     * @return bool
     */
    public static function match($pattern, $string, $options = null, $isCaseInsensitive = \false, &$matches = null)
    {
        $matches = [];
    ...
    private static function preparePatternWithFlags($pattern, $isCaseInsensitive, $options)
    {
        ...
        if (!is_string($pattern)) {
            throw new InvalidArgumentException(sprintf('Argument #1 ($pattern) must be of type string, %s given', is_object($pattern) ? get_class($pattern) : gettype($pattern)));
        }
        if ($pattern === '') {
    ...
    private static function prepareString($string, $argumentPosition)
    {
        ...
        if (!is_string($string)) {
            throw new InvalidArgumentException(sprintf('Argument #%d ($string) must be of type string, %s given', $argumentPosition, is_object($string) ? get_class($string) : gettype($string)));
        }
        return $string;
    }
    /**
    ...
    private static function prepareReplacement($replacement)
    {
        if (preg_match('/\\k\'[a-zA-Z0-9]+\'/', $replacement)) {
            throw new InvalidArgumentException('Named backreferences in the form of "\\k\'\'" are not supported by zendtech/polyfill-mb-ereg');
        }
        if (preg_match('/\\k<[a-zA-Z0-9]+>/', $replacement)) {
            throw new InvalidArgumentException('Named backreferences in the form of "\\k<>" are not supported by zendtech/polyfill-mb-ereg');
        }
    ...
    private static function normalizeMatches(array &$matches)
    {
        ...
        if (count($indexedCaptures) !== count($namedCaptures) && count($namedCaptures) > 0) {
